...
import { search as filmSearch } from '../../services/filmService'
...
const Search = () => {
    ...
    const { films, isLoading, isAllLoaded, search, sort, filter, dir, sorts, error, isInitialLoaded } = state

    const handleSearchOnBottom = useCallback(() => {
        if (!isLoading && !isAllLoaded && !error && isInitialLoaded) {
            dispatch({
                type: 'load'
            })
        }
    }, [error, isAllLoaded, isInitialLoaded, isLoading])
    ...
    useEffect(() => {
        ...
        async function getFilms() {
            await filmSearch({ ...params, limit: searchPageMaxFetchCount, searchPage: true })
                .then(res => {
                    dispatch({
                        type: 'initial-success',
